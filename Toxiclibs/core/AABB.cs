...
{
    public class AABB : Vec3D, Shape3D
    {
         ...
         */
        public static AABB getBoundingBox(List<Vec3D> points)
        {
         ...
         * @param extent
         */
        public AABB(float extent) : this(new Vec3D(), extent)
        {
        }
         ...
         * @param extent
         */
        public AABB(ReadonlyVec3D pos, float extent) : base(pos)
        {
         ...
         *            direction)
         */
        public AABB(ReadonlyVec3D pos, ReadonlyVec3D extent) : base(pos)
        {
            setExtent(extent);
        }
         ...
         */
        public bool intersectsBox(AABB box)
        {
            Vec3D t = box.sub(this);
            return Math.Abs(t.x) <= (extent.x + box.extent.x)
                    && Math.Abs(t.y) <= (extent.y + box.extent.y)
                    && Math.Abs(t.z) <= (extent.z + box.extent.z);
        }

        public Vec3D closestPointOnAABB(Vec3D p)
        {
            ...
            /*
            double dx = Math.Max(min.x - p.x, Math.Max(0, p.x - max.x));
            double dy = Math.Max(min.y - p.y, Math.Max(0, p.y - max.y));
            double dz = Math.Max(min.z - p.z, Math.Max(0, p.z - max.z));
...
*/
        ...
        public bool intersectsSphere(Vec3D c, float r)
        {
            float s, d = 0;
        ...
        /*
public bool intersectsTriangle(Triangle3D tri)
{
...
*/
        ...
        private bool planeBoxOverlap(Vec3D normal, float d, Vec3D maxbox)
        {
        ...
        private bool testAxis(float a, float b, float fa, float fb, float va,
