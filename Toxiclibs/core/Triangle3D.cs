...
namespace Toxiclibs.core
{
    public class Triangle3D : Shape3D
    {
        ...
        public static Triangle3D createEquilateralFrom(Vec3D a, Vec3D b)
        {
            ...
            c.addSelf(n.normalizeTo(dir.magnitude() * (float)Math.Sqrt(3) / 2));
            return new Triangle3D(a, b, c);
        }

        public static bool isClockwiseInXY(Vec3D a, Vec3D b, Vec3D c)
        {
            float determ = (b.x - a.x) * (c.y - a.y) - (c.x - a.x) * (b.y - a.y);
            return (determ < 0.0);
        }

        public static bool isClockwiseInXZ(Vec3D a, Vec3D b, Vec3D c)
        {
            float determ = (b.x - a.x) * (c.z - a.z) - (c.x - a.x) * (b.z - a.z);
            return (determ < 0.0);
        }

        public static bool isClockwiseInYZ(Vec3D a, Vec3D b, Vec3D c)
        {
        ...
        {
            return new Vec3D(a.x * p.getX() + b.x * p.getY() + c.x * p.getZ(), a.y * p.getX()
                    + b.y * p.getY() + c.y * p.getZ(), a.z * p.getX() + b.z * p.getY() + c.z
                    * p.getZ());
        }
        ...
        public bool isClockwiseInXY()
        {
            return Triangle3D.isClockwiseInXY(a, b, c);
        }

        public bool isClockwiseInXZ()
        {
            return Triangle3D.isClockwiseInXY(a, b, c);
        }

        public bool isClockwiseInYZ()
        {
            return Triangle3D.isClockwiseInXY(a, b, c);
        }

        private bool isSameClockDir(Vec3D a, Vec3D b, ReadonlyVec3D p, Vec3D norm)
        {
            float bax = b.x - a.x;
            float bay = b.y - a.y;
            float baz = b.z - a.z;
            float pax = p.getX() - a.x;
            float pay = p.getY() - a.y;
            float paz = p.getZ() - a.z;
